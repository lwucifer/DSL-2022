<template>
  <div class="booking-review">
    ...
    <div class="bg-white pa-16">
      ...
      <div class="mb-16 primary text-center">{{ $t('booking.thank_for_pre_order') }}</div>

      <div class="booking-actions mt-24 d-flex justify-content-end">
        <v-button variant="primary" size="lg" @click="next">{{ $t('booking.send_request') }}</v-button>
      </div>
    </div>
    ...
    <v-modal
      ...
      @ok="() => booking({ paymentType: PAYMENT_TYPE.CASH })"
...
import { TOUR_DEPARTURE_TYPE, PAYMENT_TYPE, ORDER_TYPE, PRODUCT_TYPE } from '~/utils/constants';
...
export default {
  ...
  created() {
    ...
    this.ORDER_TYPE = Object.freeze(ORDER_TYPE);
  ...
  methods: {
    ...
    ...mapActions('booking/booking', ['postBookingProduct']),
    ...
    async booking({ paymentType }) {
      const result = await this.postBookingProduct({
        ...
        orderType: ORDER_TYPE.PRE_ORDER,
      ...
      if (result.code == RES_STATUS_SUCCESS) {
        const { orderIdMask, price, currencyCode } = result.data;
        this.$router.push(this.localePath({
          path: '/booking/product-travel/pre-order/result',
          query: {
            ...this.getNextQuery(),
